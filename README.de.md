...
`hass.language` — aktuell Deutsch und Englisch, Fallback Englisch bei jeder anderen Sprache.
Auch die eigenen Texte auf der Karte (z. B. `n/v`/`n/a` bei einer nicht verfügbaren Entität)
folgen derselben Einstellung. Der Beschreibungstext im Karten-Picker steht beim Laden fest
(aus der Browsersprache), weil er gesetzt wird, bevor ein Dashboard sein `hass` bereitstellt.
...
Editor→Config-Umwandlung und die Sprachauswahl des Editors.
